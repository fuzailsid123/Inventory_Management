...
inline unsigned long basic_hash(const string& str) {
...
struct User {
    ...
    User() : id(0), passwordHash(0) {
        fill(email, email + 100, '\0');
        fill(name, name + 100, '\0');
    }
};
...
struct Order {
    ...
    Order() : id(0), itemCount(0), totalValue(0.0) {
        fill(customerName, customerName + 100, '\0');
    }
};

inline string escape_json(const string& s) {
    stringstream ss;
    for (char c : s) {
        switch (c) {
            case '"': ss << "\\\""; break;
...
inline string product_to_json(const Product& p) {
    stringstream ss;
    ss << "{";
    ss << "\"id\":" << p.id << ",";
...
inline string order_to_json(const Order& o) {
    stringstream ss;
    ss << "{";
    ss << "\"id\":" << o.id << ",";
...
string vector_to_json(const vector<T>& vec, Func to_json_func) {
    stringstream ss;
    ss << "[";
...
using ProductMinHeap = priority_queue<Product, vector<Product>, CompareProductStock>;

inline bool compareProductNameASC(const Product& a, const Product& b) { return string(a.name) <string(b.name); }
inline bool compareProductNameDESC(const Product& a, const Product& b) { return string(a.name) > string(b.name); }
