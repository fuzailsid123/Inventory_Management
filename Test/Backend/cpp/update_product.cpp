...
int main(int argc, char* argv[]) {
    ...
    double price = std::stod(argv[4]); // -1 means no change
    int quantity = std::stoi(argv[5]); // -1 means no change
    std::string category = argv[6];

    std::vector<Product> products = load_data<Product>(PRODUCTS_DB);
    Product* product_to_update = nullptr;

    // DSA: Linear search. A hash map<int, Product*> would be O(1)
    for (auto& p : products) {
    ...
    // Update fields if they were provided (not empty or -1)
